...
const EditUserInfoModal = ({ isOpen, onClose }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 modal-overlay" onClick={handleOutsideClick}>
      <div className="bg-white rounded-lg p-6 max-md:max-w-lg w-[1000px] shadow-lg relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Hourly Rate ($50/hr)</label>
            <input type="number" name="hourlyRate" value={profile.hourlyRate} onChange={handleChange} placeholder="Hourly Rate ($50/hr)" className="mt-1 px-5 py-3 tracking-wide block w-full rounded-md border border-[#00000033] shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm text-xs" />
          </div>
          <div>
        ...
        <div className="flex justify-start gap-4 mt-6">
          <button onClick={handleSave} className="px-16 py-2 bg-[#FF4C4A] text-white rounded-full shadow-sm hover:bg-[#FF4C4A] focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">Save</button>
          <button onClick={onClose} className="px-16 py-2 border border-[#00000033] bg-white rounded-full text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Cancel</button>
        </div>
      </div>
    </div>
  );
