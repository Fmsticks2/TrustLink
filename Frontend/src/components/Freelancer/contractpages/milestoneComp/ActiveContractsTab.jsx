...
import { AwaitingMilestoneItem } from '../MilestoneTracker'; // Assuming this is the correct path

const ActiveContractsTab = ({ activeContract, onShowSubmitForm, onSubmit }) => {
  ...
  return (
    <div>
      ...
      <div className="mb-6 p-4">
        ...
        {milestoneTab === 'active' && activeContract.activeMilestones.length > 0 && (
          <div className="mt-4">
            {activeContract.activeMilestones.map((milestone) => (
              <ActiveMilestoneItem
                key={milestone.id}
                milestone={milestone}
                onShowSubmitForm={onSubmit}
                onSubmit={onSubmit}
              />
            ))}
          </div>
